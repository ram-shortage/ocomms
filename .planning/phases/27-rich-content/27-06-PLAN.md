...
must_haves:
  ...
  key_links:
    ...
    - from: "src/server/socket/handlers/message.ts"
      to: "src/server/queue/link-preview.queue.ts"
...
<task type="auto">
  <name>Task 1: Wire link preview queue to message send handler</name>
  <files>src/server/socket/handlers/message.ts</files>
  <action>
Add link preview job queueing when messages are sent with URLs:
...
2. After the message is saved and before creating notifications (around line 205 after the `console.log` statement), add link preview queueing:
...
This wiring ensures:
- URLs are extracted from message content after save
- Each URL gets queued as a separate BullMQ job
- Jobs include position for ordering (LINK-02)
- Job ID includes message+URL for deduplication
- Non-blocking via fire-and-forget pattern (catch only)
  </action>
  <verify>npx tsc --noEmit src/server/socket/handlers/message.ts</verify>
  <done>Message send handler queues link preview jobs for URLs, completing the LINK-01 pipeline</done>
</task>

<task type="auto">
  <name>Task 2: Create emoji management settings page</name>
...
<task type="auto">
  <name>Task 3: Render custom emoji in message content</name>
...
- [ ] Message send handler imports extractUrls and linkPreviewQueue
- [ ] Message send handler calls linkPreviewQueue.add() for each URL
