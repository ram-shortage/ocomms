...
1. Update `src/lib/auth.ts`:
   ...
   Extend the EXISTING 'after' hook to handle '/change-password':
   - NOTE: The 'before' hook already handles '/change-password' for password complexity (line 94)
   - Add an 'after' hook check for ctx.path === "/change-password"
   - On success, call revokeAllUserSessions(userId, currentSessionId)
   - Keep current session active, revoke others
   - The path '/change-password' is already used in better-auth (see existing code at line 94)
