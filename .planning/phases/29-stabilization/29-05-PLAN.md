...
2. Open browser to status editor
3. Set a status and click Save
4. Check terminal console output for logged values
5. Run DB query to verify: `psql $DATABASE_URL -c "SELECT * FROM user_statuses ORDER BY updated_at DESC LIMIT 5"`
6. Document the root cause: action not called, action not saving to DB, or UI not refreshing
  </verify>
  <done>
