...
must_haves:
  truths:
    ...
    - "Multiple auth states exist for multi-user testing (alice.json, bob.json)"
  artifacts:
...
3. Create `e2e/tests/auth.setup.ts`:
   ...
   - Sign in as Alice (alice@demo.ocomms.local / password123)
   - Wait for redirect to workspace
   - Save storage state to `e2e/.auth/alice.json`
   - Then sign in as Bob (bob@demo.ocomms.local / password123)
   - Save storage state to `e2e/.auth/bob.json`
   - Create `e2e/.auth/user.json` as alias to alice.json for backward compatibility
