# OComms v0.6 Deployment Guide

A comprehensive guide for deploying OComms v0.6.0, a self-hosted team communication platform.
...
openssl rand -hex 32     # Use for AUDIT_LOG_SECRET
...
# Required (v0.6): AUDIT_LOG_SECRET
# Optional: SMTP_*, VAPID_*, REDIS_URL, ALLOWED_ORIGINS
...
# Generate audit log secret (required for v0.6)
...
1. `server_name your-domain.com;` (both HTTP and HTTPS blocks)
...
Wait until you see:
- `app` shows: "Ready on http://0.0.0.0:3000"
...
#### Step 5.2: Enable MFA (Recommended)
...
#### Step 5.3: Configure Email (Optional but Recommended)
...
| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `AUDIT_LOG_SECRET` | Yes | - | 32-byte hex secret for HMAC-signed audit logs |
| `ALLOWED_ORIGINS` | No | APP_URL | Comma-separated Socket.IO CORS whitelist |
| `ALLOWED_REDIRECT_DOMAINS` | No | APP_URL host | Allowed redirect domains (prefix with . for subdomain) |
...
# SECURITY (v0.6 required)
# ===================
AUDIT_LOG_SECRET=0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef
...
| Command | Description |
|---------|-------------|
| `node migrate.mjs` | Apply pending migrations (production) |
...
- **Two-Factor:** TOTP secrets and backup codes
- **Password History:** Previous passwords for reuse prevention
- **Workspace Join Requests:** Request-based workspace joining
- **User Sidebar Preferences:** Per-user sidebar layout
...
If using the wrong command:

```bash
# Correct command for production:
docker compose exec app node migrate.mjs

# NOT this (requires dev dependencies):
...
# Run backup
docker compose exec -T db pg_dump -U postgres ocomms > ./backups/ocomms-$(date +%Y%m%d-%H%M%S).sql
```
...
- [ ] Secrets generated (auth secrets, VAPID keys, audit log secret)
...
- **Upgrade Guide:** See [upgrade-v0.5-to-v0.6.md](./upgrade-v0.5-to-v0.6.md)
